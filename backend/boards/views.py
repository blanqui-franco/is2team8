    ...
    def get_queryset(self, *args, **kwargs):
        ...
        if sort == "recent":
            ...
            #board_ids = r.zrange(redis_key, 0, 3, desc=True)
              # Recupera los tableros más recientes visualizados por el usuario desde el modelo RecentlyViewedBoard
            recent_boards = RecentlyViewedBoard.objects.filter(user=self.request.user).order_by('-viewed_at')[:3]
            board_ids = [recent.board.id for recent in recent_boards]
...
class BoardDetail(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        ...
        recent_view, created = RecentlyViewedBoard.objects.get_or_create(
            user=self.request.user, board_id=board_id
        )
        recent_view.viewed_at = timezone.now()
        recent_view.save()
